...
def tn2v(main_directory, project_name, embedding_dimension, n2v_param_array, l0_array, l1_array, l2_array, eta_array, LEN,
    ...
    lift_array=None, target_pd=None, initial_W1=None, pointcloud_data_save=None, pointcloud_vf_save=None, reciprocal_gamma=0.0001,
    reciprocal_nu=1.0):
...
class TN2V:
    ...
    def generate_n2v_info(self):
        ...
        **Does not support the option to stay put. (I.e., self-loops are ignored.)
        ...
        for i in range(self.target_correlation_matrix.shape[0]):
            for j in range(i+1,self.target_correlation_matrix.shape[0]):
                self.edgedict[str(i)+'-'+str(j)] = self.target_correlation_matrix[i][j]
    ...
    def one_step_vectors(self,x_in,x_E):
        ...
        '''
        n,m = self.W1.shape
        x_in = np.array(x_in).reshape(n,1) # (n x 1): this is x_in as a column.
    ...
    def print_vf_bc(self,i,save=True,wma=0):
        ...
        plt.legend(loc=0)
        if save:
            plt.savefig('./tn2v_output/'+self.name+'/print_vf_bc_'+str(self.complete_counter).zfill(12)+'_deg'+str(i)+'.png')
            plt.close()
        else:
            plt.show()
    ...
    def print_vf_pc(self):
        ...
        plt.savefig('./tn2v_output/'+self.name+'/print_vf_pc_'+str(self.epoch_counter).zfill(12)+'.png')
        plt.close()
    ...
    def train(self,
        ...
        for unrelated_counter in range(epochs):
            ...
            if i in pointcloud_data_save:
                pd.DataFrame(self.W1).to_csv('./tn2v_output/'+project_name+'w1_at_'+str(i).zfill(12)+'.csv')
    ...
    def get_next_random_node(self,p,q,i,prev=None):
        ...
        j = np.random.choice(candidates, p=probabilities)
        return j
